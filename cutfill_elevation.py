...
def check_direction(coord,Direction):
    ...
    for i in range(len(transformed)-1):
        ...
        if direction==Direction:
            ...
            op.append([transformed[i+1][1],transformed[i-1][0]])
...
class featurecalculation:
    def features(self,filename):
        ...
        for divo in range(division):
            # elem={}
            # elem["name"] = names[divo]
            # elem["cut"] = result[divo][0]
            # elem["fill"] = result[divo][1]
        ...
        with open("elevation_output.json", "w") as outfile:
    ...
    def calc(self,div):
        ...
        start,end=check_direction(my_cord,direction)
        start_line=LineString(start)
        p1=Point(start[0])
        p2=Point(end[0])
        d=p1.distance(p2)
        slope=(base_station_ht-z)/d
